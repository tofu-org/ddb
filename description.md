...
```mermaid
graph TD
    subgraph "Презентационный уровень"
        A[HTML-шаблоны, CSS<br>веб-браузер]
    end
    
    subgraph "Уровень приложения"
        B[Flask, маршруты<br>контроллеры]
    end
    
    subgraph "Уровень данных"
        C[PostgreSQL кластер]
        
        subgraph "Administration (Master)"
            D[Port 5000]
        end
        
        subgraph "Worker nodes"
            E[Shop1<br>Port 5001]
            F[Shop2<br>Port 5002]
            G[WH1/2<br>Port 5011]
        end
    end
    
    A -- HTTP-запросы/ответы --> B
    B -- ORM (SQLAlchemy) --> C
    D -- Логическая репликация --> E
    D -- Логическая репликация --> F
    D -- Логическая репликация --> G
...
```mermaid
flowchart TD
    A[Administration<br>Центральный узел<br>Port 5000]
    
    A --> B[Shop 1<br>Port 5001]
    A --> C[Shop 2<br>Port 5002]
    A --> D[Warehouse 1<br>Port 5011]
    A --> E[Warehouse 2<br>Port 5012]
```
